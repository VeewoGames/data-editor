...
### 8. 个人模式直存成功后，内存中的 shared views 快照必须同步提交给后续写入入口

后续真实 Chrome 复现又补出了一条 durable 边界：在 `personal mode` 下，shared view 的 direct save 不能只把数据写进 `shared-views.json`，还必须同步更新后续写入会读取的内存快照。

当前实现里，这个内存快照具体是 `src/App.tsx` 内被 `enqueueSharedViewDirectSave(...)` 读取的 `sharedViewsConfigRef.current`。如果 icon 更新刚保存成功，而这个 ref 仍停留在旧值，用户紧接着再做 top-level reorder，第二次 direct save 就可能把刚保存的新 icon 覆盖回旧值。

因此，这里的长期协议不是“保存成功后迟早会通过 React state 同步回来”，而是：

- 任何 shared views config 成功保存后
- 都要同步提交 React state 与后续 direct save 读取的 ref 快照
- 不能把 ref 的一致性托付给晚一拍的 `useEffect`

当前落地入口是 `commitSharedViewsConfig(...)`，后续只要 personal mode 仍保留 direct save，就应继续保持这条同步合同。
...
- 个人模式的连续直接保存不仅要串行，还要保证每次成功保存后同步刷新后续写入所读的内存快照，否则会出现“前一次已写入、后一次又覆盖回旧值”的时序回退。
...
`sharedViewPublishVisible`、`sharedViewPublishEnabled`、`enqueueSharedViewDirectSave`、`sharedViewsConfigRef.current`、`commitSharedViewsConfig`、`saveSharedViewDraftsToConfig`、`shared-views.json`、`个人模式`、`团队模式`、`显式发布`
