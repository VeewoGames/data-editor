...
const profile = stableClusterProfiles.text;
export const promoteStablePrefixBatch11Suggestion = (suggestion) => promoteStableClusterSuggestion(suggestion, profile);
export const promoteStreamlineStablePrefixBatch11Suggestions = (payload) => promoteStableClusterSuggestions(payload, profile);
...
async function main(argv){const p=parseCliArgs(argv); if(!p.suggestionsPath) throw new Error("Usage: node scripts/streamline-export/promote-streamline-stable-prefix-batch11.mjs <suggestionsPath> [--output <path>] [--promoted-output <path>]"); const r=await writeArtifacts({suggestionsPath:p.suggestionsPath,outputPath:p.outputPath,promotedSuggestionsPath:p.promotedSuggestionsPath}); console.log(JSON.stringify(r,null,2));}
if (typeof process !== "undefined" && process.argv?.[1]?.endsWith("promote-streamline-stable-prefix-batch11.mjs")) { main(process.argv).catch((error)=>{console.error(error instanceof Error ? error.message : String(error)); process.exitCode=1;});}
