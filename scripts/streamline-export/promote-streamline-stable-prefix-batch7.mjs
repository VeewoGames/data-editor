...
const profile = stableClusterProfiles.time;

export function promoteStablePrefixBatch7Suggestion(suggestion) {
  return promoteStableClusterSuggestion(suggestion, profile);
}

export function promoteStreamlineStablePrefixBatch7Suggestions(payload) {
...
async function main(argv) {
  ...
  if (!parsed.suggestionsPath) {
    throw new Error("Usage: node scripts/streamline-export/promote-streamline-stable-prefix-batch7.mjs <suggestionsPath> [--output <path>] [--promoted-output <path>]");
  }
...
if (typeof process !== "undefined" && process.argv?.[1]?.endsWith("promote-streamline-stable-prefix-batch7.mjs")) {
