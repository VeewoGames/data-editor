...
const profile = stableClusterProfiles.warning;

export function promoteStablePrefixBatch5Suggestion(suggestion) {
  return promoteStableClusterSuggestion(suggestion, profile);
}

export function promoteStreamlineStablePrefixBatch5Suggestions(payload) {
...
async function main(argv) {
  ...
  if (!parsed.suggestionsPath) {
    throw new Error("Usage: node scripts/streamline-export/promote-streamline-stable-prefix-batch5.mjs <suggestionsPath> [--output <path>] [--promoted-output <path>]");
  }
...
if (typeof process !== "undefined" && process.argv?.[1]?.endsWith("promote-streamline-stable-prefix-batch5.mjs")) {
