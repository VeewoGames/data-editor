export function buildEntryActionProposalPrompt({ skillPath, skillContent, handoff }) {
  ...
  return [
    ...
    "proposal 顶层必须且只能包含这些键：version, runId, actionId, sourcePath, canonicalFileKey, collectionPath, rowId, baseDocumentEtag, automationProfileEtag, authorityDigest, fencingToken, changes, textArtifact, summary。",
    "禁止返回 status、action、target、error 等额外顶层键。",
    "version 必须为 2；runId、actionId、sourcePath、canonicalFileKey、collectionPath、rowId、baseDocumentEtag、automationProfileEtag、authorityDigest、fencingToken 必须从 handoff 对应字段原样复制。",
    ...
    "textArtifact 若无授权产物必须为 null；若有授权产物，必须且只能包含 id, path, beforeExists, beforeDigest, afterContent, afterDigest，并从 proposalContract.textArtifact 原样复制 id、path、beforeExists、beforeDigest。",
