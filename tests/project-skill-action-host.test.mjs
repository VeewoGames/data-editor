...
test("project-skill host runs the cli from an isolated output root", async (t) => {
  ...
  const fake = path.join(output, "exec");
  ...
  const code = await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.resolve("scripts/run-project-skill-action-host.mjs"), "--codex", process.execPath, "--input-root", input, "--output-root", output, "--prompt", prompt, "--reply", reply, "--events", events, "--diagnostics", diagnostics, "--model", "fixture", "--reasoning", "low", "--verbosity", "low"], { cwd: root, env: process.env });
    child.once("error", reject); child.once("exit", resolve);
  });
  assert.equal(code, 0);
  const result = JSON.parse(await readFile(reply, "utf8"));
  assert.equal(result.resultOnly, true);
  assert.equal(path.resolve(result.cwd), path.resolve(output));
