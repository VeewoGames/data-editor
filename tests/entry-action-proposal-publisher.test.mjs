...
test("proposal publishes atomically only after success and validation", async () => {
  ...
  try {
    const result = await publishEntryActionProposal({ directory: root, runId: proposal.runId, exitCode: 0, proposal });
    assert.deepEqual(JSON.parse(await readFile(result.path, "utf8")), proposal);
  } finally {
